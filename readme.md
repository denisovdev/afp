...
```json
{
  ...
  "device_cluster_id": 42,
  "match_type": "device_id",
  "score": 0.98,
  "is_multi_account_suspected": true,
  "linked_accounts_count": 2,
  "device_linked_accounts_count": 3
}
```

| Поле                           | Описание                                                                 |
|--------------------------------|--------------------------------------------------------------------------|
| `browser_profile_id`           | ID профиля браузерного окружения                                         |
| `device_cluster_id`            | ID кластера физического устройства                                       |
| `match_type`                   | Способ сопоставления: `device_id`, `hard_fp`, `soft_fp`, `candidate`, `new_profile` |
| `score`                        | Similarity score [0, 1]                                                  |
| `is_multi_account_suspected`   | `true` если >1 аккаунта на одном browser_profile                        |
| `linked_accounts_count`        | Количество аккаунтов на этом browser_profile                             |
| `device_linked_accounts_count` | Количество аккаунтов на всех browser_profile одного device_cluster       |

---

## Сценарии проверки

> Кейсы предназначены для последовательного выполнения на чистой БД (`docker compose down -v && docker compose up --build`). Все SHA-256 хеши и UUID реалистичны — 64 hex-символа / формат v4.
...
```bash
curl -s -X POST http://localhost:8080/api/v1/fingerprints/resolve \
...
```json
{
  ...
  "score": 0,
  "is_multi_account_suspected": false,
...
Что произошло:
- `browser_profile_id: 2` — создан новый профиль (другой браузер → другие device_id, canvas, webgl)
- `device_cluster_id: 1` — **тот же кластер устройства** (hardware_fp совпал: cpu_cores=16, screen 2560×1440, pixel_ratio=1, color_depth=24, max_touch_points=0, Win32, Europe/Moscow)
- `linked_accounts_count: 1` — на этом browser_profile пока один аккаунт
- `device_linked_accounts_count: 2` — на уровне устройства 2 уникальных аккаунта (1001 + 2002)
...
Аккаунт 2002 (который ранее использовался на Windows-десктопе в кейсе 2) теперь заходит с MacBook. `device_id` тот же что в кейсе 4 (тот же Chrome на том же MacBook) → матч по device_id. Система фиксирует мультиаккаунтинг: на этом browser_profile уже был аккаунт 1001.
...
Аккаунт 2002 теперь привязан к browser_profile 1 (Windows-десктоп, Chrome) и browser_profile 3 (MacBook, Chrome) — мультиаккаунтинг виден на обоих устройствах.
...
internal/http/               — HTTP-хендлеры (chi router, CORS middleware)
...
web/                         — JS-клиент сбора fingerprint
```
